...
def landing_html() -> str:
    ...
    paid_line = (
        "x402 exact 0.003 USDC on Base"
        if payment_flags_on()
        else "currently free (payment flags off)"
    )
...
<head>
  ...
  <title>Agent JSON Reliability</title>
</head>
<body>
  <h1>Agent JSON Reliability</h1>
  <p>Inspect, safely repair and validate malformed JSON generated by software and AI agents.</p>
  <p>No LLM. Deterministic. No account.</p>
  <ul>
    <li>Free: inspect, validate, repair, MCP</li>
    <li>Paid hosted reliability endpoint: POST /v1/json/reliable — {escape(paid_line)}</li>
  </ul>
  {notice}
